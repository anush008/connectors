...
async def probe_report(
    ...
    """
    # Yesterday: a whole elapsed day, always a valid thing to ask for. The
    # response may hold anything up to the row cap, so only the first row is
    # read — enough to confirm the query ran and returned the expected columns.
    # With no rows, the generator instead runs to completion and its envelope
    # check reports any `errors` Criteo returned.
    probe_day = floor_to_day(datetime.now(tz=UTC)) - TICK

    async with aclosing(
        _request_report_window(
